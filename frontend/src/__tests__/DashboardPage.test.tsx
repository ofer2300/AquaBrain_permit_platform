...
describe('DashboardPage', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // Removed fake timers - using real timers for reliability
  });
...
await waitFor(() => {
        const trendElements = screen.getAllByText(/מהחודש הקודם/i);
...
await waitFor(() => {
        const projectElements = screen.getAllByText(/מתחם מגורים חדש/i);
...
await waitFor(() => {
        const projectNameCol = screen.getAllByText(/שם הפרויקט/i);
        const cityCol = screen.getAllByText(/עיר/i);
        const statusCol = screen.getAllByText(/סטטוס/i);
        const dateCol = screen.getAllByText(/תאריך/i);
        expect(projectNameCol.length).toBeGreaterThan(0);
        expect(cityCol.length).toBeGreaterThan(0);
        expect(statusCol.length).toBeGreaterThan(0);
        expect(dateCol.length).toBeGreaterThan(0);
      });
    });
...
await waitFor(() => {
        const project1 = screen.getAllByText(/מתחם מגורים חדש/i);
        const project2 = screen.getAllByText(/בניין משרדים/i);
        expect(project1.length).toBeGreaterThan(0);
        expect(project2.length).toBeGreaterThan(0);
      });
    });
...
await waitFor(() => {
        const city1 = screen.getAllByText(/תל אביב/i);
        const city2 = screen.getAllByText(/הרצליה/i);
        expect(city1.length).toBeGreaterThan(0);
        expect(city2.length).toBeGreaterThan(0);
      });
    });
...
await waitFor(() => {
        const status1 = screen.getAllByText(/פעיל/i);
        const status2 = screen.getAllByText(/הושלם/i);
...
await waitFor(() => {
        const projectNames = screen.getAllByText(/מתחם מגורים חדש/i);
...
await waitFor(() => {
        const approved = screen.getAllByText(/אושר/i);
        const rejected = screen.getAllByText(/נדחה/i);
...
await waitFor(() => {
        const passed = screen.getAllByText(/עבר/i);
        const failed = screen.getAllByText(/נכשל/i);
...
await waitFor(() => {
        const numbers = screen.getAllByText(/50/i);
...
await waitFor(() => {
        const status = screen.getAllByText(/פעיל/i);
        const result = screen.getAllByText(/עבר/i);
...
await waitFor(() => {
        const tlv = screen.getAllByText(/תל אביב/i);
        const herzl = screen.getAllByText(/הרצליה/i);
        expect(tlv.length).toBeGreaterThan(0);
        expect(herzl.length).toBeGreaterThan(0);
      });
    });
  });
...
await waitFor(() => {
        const badges = screen.getAllByText(/פעיל/i);
